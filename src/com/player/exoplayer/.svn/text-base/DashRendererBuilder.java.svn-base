...
import com.player.exoplayer.LukupPlayer.RendererBuilder;
import com.player.exoplayer.LukupPlayer.RendererBuilderCallback;
...
public class DashRendererBuilder implements RendererBuilder,
  ...
  public DashRendererBuilder(String userAgent, String url, String contentId,
	 ...
	 Log.i("DashRendererBuilder", "Inside DashRendererBuilder");
  ...
  @Override
  public void buildRenderers(LukupPlayer player, RendererBuilderCallback callback) {
    this.player = player;
  ...
  public void onManifest(String contentId, MediaPresentationDescription manifest) {
    ...
    boolean hasContentProtection = true;
...
//          ? new DebugTrackRenderer(debugTextView, videoRenderer, videoSampleSource) : null;
    ...
    if (textChunkSourceList.isEmpty()) {
      ...
      SampleSource textSampleSource = new ChunkSampleSource(textChunkSource, loadControl,
          ...
          LukupPlayer.TYPE_TEXT);
      textRenderer = new TextTrackRenderer(textSampleSource, player, mainHandler.getLooper(),
          new TtmlParser(), new WebvttParser());
    }

    // Invoke the callback.
    String[][] trackNames = new String[LukupPlayer.RENDERER_COUNT][];
    trackNames[LukupPlayer.TYPE_AUDIO] = audioTrackNames;
    trackNames[LukupPlayer.TYPE_TEXT] = textTrackNames;

    MultiTrackChunkSource[] multiTrackChunkSources =
        new MultiTrackChunkSource[LukupPlayer.RENDERER_COUNT];
    multiTrackChunkSources[LukupPlayer.TYPE_AUDIO] = audioChunkSource;
    multiTrackChunkSources[LukupPlayer.TYPE_TEXT] = textChunkSource;

    TrackRenderer[] renderers = new TrackRenderer[LukupPlayer.RENDERER_COUNT];
    renderers[LukupPlayer.TYPE_VIDEO] = videoRenderer;
    renderers[LukupPlayer.TYPE_AUDIO] = audioRenderer;
    renderers[LukupPlayer.TYPE_TEXT] = textRenderer;
   // renderers[LukupPlayer.TYPE_DEBUG] = debugRenderer;
  ...
  private static class V18Compat {
    ...
    public static Pair<DrmSessionManager, Boolean> getDrmSessionManagerData(LukupPlayer player,
